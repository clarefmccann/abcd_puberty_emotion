...
vars_to_standardize <- c("growth_spurt", "body_hair", "skin_change", "breast_develop", "menarche", "age", "covid_date", "caffeine", "active", "sample_collection_time", "midnight_to_start", "time_to_freezer", "dhea_norm", "tst_norm")
scaled_puberty_female <- puberty_female
scaled_puberty_female[vars_to_standardize] <- scale(scaled_puberty_female[vars_to_standardize])

vars_to_standardize <- c("growth_spurt", "body_hair", "skin_change", "voice_deep", "face_hair", "age", "covid_date", "caffeine", "active", "sample_collection_time", "midnight_to_start", "time_to_freezer", "dhea_norm", "tst_norm")
scaled_puberty_male <- puberty_male
scaled_puberty_male[vars_to_standardize] <- scale(scaled_puberty_male[vars_to_standardize])

## removing confounding effects 

model_f_dhea <- lmer(dhea_norm ~ caffeine +  sample_collection_time  + midnight_to_start + time_to_freezer + covid_date + (1 | site_id) + (1 | id), data = scaled_puberty_female)

model_f_tst <- lmer(tst_norm ~ caffeine +  sample_collection_time + midnight_to_start +  time_to_freezer + covid_date + (1 | site_id) + (1 | id), data = scaled_puberty_female)

model_m_dhea <- lmer(dhea_norm ~ caffeine + sample_collection_time  + midnight_to_start + time_to_freezer + covid_date + (1 | site_id) + (1 | id), data = scaled_puberty_male)

model_m_tst <- lmer(tst_norm ~ caffeine + sample_collection_time + midnight_to_start + time_to_freezer + covid_date + (1 | site_id) + (1 | id), data = scaled_puberty_male)

## adding residuals to dataframes 

scaled_puberty_female$dhea_unconfounded <- residuals(model_f_dhea)
scaled_puberty_female$tst_unconfounded <- residuals(model_f_tst)
scaled_puberty_male$dhea_unconfounded <- residuals(model_m_dhea)
scaled_puberty_male$tst_unconfounded <- residuals(model_m_tst)


scaled_puberty_female$"id-wave" <- paste(scaled_puberty_female$id, scaled_puberty_female$wave, sep = "-")
scaled_puberty_male$"id-wave" <- paste(scaled_puberty_male$id, scaled_puberty_male$wave, sep = "-")

scaled_puberty_female <- scaled_puberty_female %>% 
  rename("pds" = "pds_f",
         "sex" = "sex.x") %>% 
...
scaled_puberty_male <- scaled_puberty_male %>% 
  rename("pds" = "pds_m",
         "sex" = "sex.x") %>% 
...
write.csv(scaled_puberty_female, file = "data/whole_abcd_puberty_female.csv")
write.csv(scaled_puberty_male, file = "data/whole_abcd_puberty_male.csv")
...
demo_ids <- left_join(whole, demo, by = "id") %>% 
  filter(!is.na(white),
         !is.na(black),
         !is.na(aina),
         !is.na(an),
...
multi_race <- multi_race[,c(2, 23:39)]

multi_race <- multi_race %>% 
  pivot_longer(cols = 2:18, names_to = "race", values_to = "yes_no") %>% 
  filter(yes_no == 1)
