...
def remove_regression_to_mean_effect(y_train, y_train_pred, y_test, y_test_pred):
    # Check if y_train_pred and y_test_pred are not None before performing calculations
    if y_train_pred is None or y_test_pred is None:
        raise ValueError("y_train_pred and y_test_pred must have valid values")

    # Perform the regression to mean effect calculation
...
def run_predictions(filename, dataname, predictors, to_predict, group_by=None, subject_col=None,
                    runmethod="svrrbf", n_splits=1, train_size=0.7, random_state=0, testindividual_predictors=False,
    ...
    # remove nans
    nanmask = mydf[all_features].isna().any(axis=0)
    mycleandf = mydf.dropna(subset=all_features).copy()
    ...
    model = None
    yt_pred = None  # Initialize yt_pred with None
    y_pred = None  # Initialize y_pred with None
